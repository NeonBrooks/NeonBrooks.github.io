...
// Create some buttons
		...
		button[i][2].setAttribute('fill','orange');
		}
		// write +5 on a bead	
		var sign = new Array();
		sign[0] = new Array();
...
// 			alert(sign[i][j].textContent)
		...
		sign[i][3] = createText(26 + 32 * i, 116 + 13 * (3-1), "-1", i, 3);
		}	
		...
		for (var i = 0; i < 4; i += 1) {
			for (var j = 0; j < 3; j +=1){
			createTb(32 + 32 * i, 108 + 13*j, i, j,1);
					}
		}
		
		//ONE MORE BUTTON
		button[4][1] = createReset(80, 150, 100, 1);
// 		alert(button[10][10])
		createText(70, 160, "Reset", 10,10);
		var reset = createReset(80, 150, 100,0, 4, 1);
...
//           	selButton.setAttribute('fill','gray');
     ...
     selSign = sign[selButton._row][selButton.column+1]
        if(selSign.textContent == "+5"){//top row, add 5
        	...
        	// find first bead that is out of play
        	for (var i = 0; i < 4; i += 1) { 
        		if (lower[selButton._row][i].isIn == 0){
        		selBead = lower[selButton._row][i];
        		break	;
        			}
        		}
        	...
        	setY(selBead, 37 + (selBead.column * 15), 'up');
        	selBead.isIn = 1;
        	selButton.setAttribute('fill','green');
//         	
			}
		...
		if (lower[selButton._row][3].isIn == 1){
			button[selButton._row][1].setAttribute('fill','orange');
		}else{
			button[selButton._row][1].setAttribute('fill','green');
		}
	
	    if (lower[selButton._row][0].isIn == 0){
        	button[selButton._row][2].setAttribute('fill','orange');
        }else{
			button[selButton._row][2].setAttribute('fill','green');
		}
