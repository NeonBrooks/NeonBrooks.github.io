...
// Create some buttons: now need 2 rows
		...
		for (var i = 0; i < 4; i += 1) {
			...
			button[i][0][1] = createButton(32 + 32 * i, 108, i, 0,1);
			for (var j = 1; j < 5; j +=1){
			button[i][j] = new Array();
			button[i][j][1] = createButtonMini(32 + 32 * i, 108 + 13*j, i, j,1);
			button[i][j][2] = createButtonMini(48 + 32 * i, 108 + 13*j, i, j,2);
		...
		for (var i = 0; i < 4; i += 1) {
			sign[i][0] = new Array();
			sign[i][0][1] = createText(26 + 32 * i, 116 , "+5",i,0);
			for (var j = 1; j < 5; j += 1){
			sign[i][j] = new Array();
			sign[i][j][1] = createText(20 + 32 * i, 116 + 13 * (j), "+" + j,i,j);
			sign[i][j][2] = createText(36 + 32 * i, 116 + 13 * (j), "-" + j,i,j);
		}
		}	
		...
		for (var i = 0; i < 4; i += 1) {
				// tb[i][0] = new Array();
// 				tb[i][0][1] = 
			createTb(32 + 32 * i, 108, i, 0,1);
			for (var j = 1; j < 5; j +=1){
			createTbMini(32 + 32 * i, 108 + 13*j, i, j,1);
			createTbMini(48 + 32 * i, 108 + 13*j, i, j,2);
					}
		}
		
		//ONE MORE BUTTON
		button[4][1] = new Array()
		button[4][1][1] = createReset(80, 175, 100, 1, 4, 1, 1);
		createText(70, 185, "Reset", 10,10);
		var reset = createReset(80, 175, 100,0, 4, 1, 1);
...
//           			button[i][j].setAttribute('fill','green');
//           			}
//           			}
...
//Auxiliary functions
	...
	function createText(x, y, val, row, column){
		...
		function createReset(x1, y1, width, opacity, row, column, b12)
		{
		var el = document.createElementNS(svgNS, 'rect');
		...
		for (var i = 0, len = beads[x].length; i < len; i += 1) { //FOR EVERY BEAD
			...
			if (y >= y2 - 15 && y <= y2 + 15) { //increasing this made all the beads come together! //if they are within 5 of each other, move them both
				...
				if (bead._index > bead2._index) { 
					if (!setY(bead2, y - 15, 'up', 1)) { //this is a recursive thing: 
				...
				// lower
					...
					if (!setY(bead2, y + 15, 'down', 1)) { //same for down
						return false;
					}else{
					bead2.isIn = -1;
					}
